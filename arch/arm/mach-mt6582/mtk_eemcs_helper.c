...
static int get_eemcs_dfo_setting(char item[], unsigned int *val)
{
	...
	for (i=0; i<(sizeof(eemcs_dfo_setting)/sizeof(dfo_item_t)); i++) {
		...
		if(!strcmp(eemcs_name, item)) {
			printk("[EEMCS]Get DFO:%s:0x%08X\n", eemcs_name, eemcs_value);
			*val = (unsigned int)eemcs_value;
			return 0;
		}
	}
	printk("[EEMCS]MD%d: DFO:%s not found\n", i+1, item);
	return -1;
}
...
int parse_ext_meta_md_setting(unsigned char args[])
{
  ...
  int  active_id =  -1;
	...
	if(!(md_active_setting & MD5_SETTING_ACTIVE)) {
		printk("[EEMCS]META EXT MD setting not found [%d][%d]\n", args[0], args[1]);
  } else {
    active_id = MD_SYS5;
  ...
  switch(active_id) {
    case MD_SYS5:
	    ...
	    printk("[EEMCS]META EXT MD type:%d\n", ext_md_support[active_index]);
      break;
  }
	return 0;	
}
...
unsigned int set_ext_modem_support(int md_id, int md_type){
#if defined(DFO_FEATURE_EN)
    printk("[EEMCS] set_ext_modem_support MD %d Type %d", md_id, md_type);
    switch(md_id) {
  	case MD_SYS5:
        if (md_type >= modem_lwg &&  md_type <= modem_ltg){
            ...
            printk("[EEMCS]error: set_modem_support fail(md:%d, md_type:%d)!\n", md_id, md_type);
            return -1;
        }
  		break;
  	default:
    ...
    printk("[EEMCS] set_ext_modem_support DFO not support");
    return -1;
...
unsigned int get_ext_md_mem_start_addr(int md_id){  
    ...
    switch(md_id) {
  	case MD_SYS5:
  		...
  		break;
  	default:
        printk("[EEMCS HELPER]get_ext_md_mem_start_addr request MD ID %d not supported\n", md_id);
  		break;
	}
...
unsigned int get_ext_md_mem_size(int md_id){
    ...
    switch(md_id) {
  	case MD_SYS5:
  		...
  		break;
  	default:
        printk("[EEMCS HELPER]get_ext_md_mem_size request MD ID %d not supported\n", md_id);
  		break;
	}
...
int clear_md_region_protection(int md_id)
{
	...
	printk("[EEMCS HELPER] Clear MD%d region protect...\n", md_id+1);
	switch(md_id)
	{
		case MD_SYS5:
			rom_mem_mpu_id = 1; //0;
		...
		default:
			printk("[EEMCS HELPER] [error]md id(%d) invalid when clear MPU protect\n", md_id+1);
			return -1;
	}
	
	printk("[EEMCS HELPER] Clear MPU protect MD%d ROM region<%d>\n", md_id+1, rom_mem_mpu_id);
	...
	printk("[EEMCS HELPER] Clear MPU protect MD%d R/W region<%d>\n", md_id+1, rw_mem_mpu_id);
	emi_mpu_set_region_protection(0,		  			/*START_ADDR*/
								  0,       				/*END_ADDR*/
...
int eemcs_register_sys_msg_notify_func(int md_id, int (*func)(int, unsigned int, unsigned int))
{
	...
	if( md_id >= MAX_EXT_MD_NUM ) {
		printk("[eemcs/ctl] (0)register_sys_msg_notify_func fail: invalid md id(%d)\n", md_id+1);
		return E_PARAM;
	}

	if(eemcs_sys_msg_notify_func[md_id] == NULL) {
		eemcs_sys_msg_notify_func[md_id] = func;
	} else {
		printk("[eemcs/ctl] (%d)eemcs_sys_msg_notify_func fail: func registered!\n", md_id+1);
	}
...
int eemcs_notify_md_by_sys_msg(int md_id, unsigned int msg, unsigned int data)
{
	...
	if(md_id >= MAX_EXT_MD_NUM) {
		printk("[eemcs/ctl] (0)notify_md_by_sys_msg: invalid md id(%d)\n", md_id+1);
		return E_PARAM;
	}

	func = eemcs_sys_msg_notify_func[md_id];
	if(func != NULL) {
		ret = func(md_id, msg, data);
	} else {
		ret = E_NO_EXIST;
		printk("[eemcs/ctl] (%d)notify_md_by_sys_msg fail: func not register!\n", md_id+1);
	}
...
int eemcs_register_ccci_sys_call_back(int md_id, unsigned int id, eemcs_sys_cb_func_t func)
{
	...
	if( md_id >= MAX_EXT_MD_NUM ) {
		printk("[eemcs/ctl] (0)register_sys_call_back fail: invalid md id(%d)\n", md_id+1);
		return E_PARAM;
	}

	if((id >= 0x100)&&((id-0x100) < MAX_KERN_API)) {
		info_ptr = &(eemcs_sys_cb_table_100[md_id][id-0x100]);
	} else if((id >= 0x1000)&&((id-0x1000) < MAX_KERN_API)) {
		info_ptr = &(eemcs_sys_cb_table_1000[md_id][id-0x1000]);
	} else {
		printk("[eemcs/ctl] (%d)register_sys_call_back fail: invalid func id(0x%x)\n", md_id+1, id);
		return E_PARAM;
	}
	...
	if(info_ptr->func == NULL) {
		...
		printk("[eemcs/ctl] (%d)register_sys_call_back fail: func(0x%x) registered!\n", md_id+1, id);
...
void eemcs_exec_ccci_sys_call_back(int md_id, int cb_id, int data)
{
	...
	if(md_id >= MAX_EXT_MD_NUM) {
		printk("[eemcs/ctl] (0)exec_sys_cb fail: invalid md id(%d) \n", md_id+1);
		return;
	}

	id = cb_id & 0xFF;
	if(id >= MAX_KERN_API) {
		printk("[eemcs/ctl] (%d)exec_sys_cb fail: invalid func id(0x%x)\n",  md_id+1, cb_id);
		return;
	}

	if ((cb_id & (0x1000|0x100))==0x1000) {
		curr_table = eemcs_sys_cb_table_1000[md_id];
	} else if ((cb_id & (0x1000|0x100))==0x100) {
		curr_table = eemcs_sys_cb_table_100[md_id];
	} else {
		printk("[eemcs/ctl] (%d)exec_sys_cb fail: invalid func id(0x%x)\n",  md_id+1, cb_id);
		return;
	}
	...
	if(func != NULL) {
		...
		printk("[eemcs/ctl] (%d)exec_sys_cb fail: func id(0x%x) not register!\n", md_id+1, cb_id);
	}
}
...
int eemcs_register_ccci_kern_func_by_md_id(int md_id, unsigned int id, eemcs_kern_cb_func_t func)
{
	...
	if((id >= MAX_KERN_API) || (func == NULL) || (md_id >= MAX_EXT_MD_NUM)) {
		printk("[eemcs/ctl] (0)register kern func fail: md_id:%d, func_id:%d!\n", md_id+1, id);
		return E_PARAM;
	}
	...
	if(info_ptr->func == NULL) {
		...
		printk("[eemcs/ctl] (%d)register kern func fail: func(%d) registered!\n", md_id+1, id);
...
int eemcs_exec_ccci_kern_func_by_md_id(int md_id, unsigned int id, char *buf, unsigned int len)
{
	...
	if(md_id >= MAX_EXT_MD_NUM) {
		printk("[eemcs/ctl] (0)exec kern func fail: invalid md id(%d)\n", md_id+1);
		return E_PARAM;
	}
	
	if(id >= MAX_KERN_API) {
		printk("[eemcs/ctl] (%d)exec kern func fail: invalid func id(%d)!\n", md_id, id);
		return E_PARAM;
	}
	...
	if(func != NULL) {
		...
		if(eemcs_kern_func_err_num[md_id][id] < SHOW_WARNING_NUM) {
			...
			printk("[eemcs/ctl] (%d)exec kern func fail: func%d not register!\n", md_id+1, id);
		}
	}
...
void eemcs_register_suspend_notify(int md_id, unsigned int id, void (*func)(int))
{
	if((id >= MAX_SLEEP_API) || (func == NULL) || (md_id >= MAX_EXT_MD_NUM)) {
		printk("[eemcs/ctl] invalid suspend parameter(md:%d, cmd:%d)!\n", md_id, id);
	}
...
{
	if((id >= MAX_SLEEP_API) || (func == NULL) || (md_id >= MAX_EXT_MD_NUM)) {
		printk("[eemcs/ctl] invalid resume parameter(md:%d, cmd:%d)!\n", md_id, id);
	}
